...
|---|---|---|
| `docs` | getText, create, writeText, getSuggestions, replaceText, formatText | Document reads return text; writes return bounded status/IDs. |
...
3. Extend caps only with an explicit operation contract: bounded default, explicit continuation token, and a caller-visible way to request more data. Never silently truncate content or destructive operations.
