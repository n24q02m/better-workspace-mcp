...
Ported from the sibling MCP servers (better-email-mcp, better-notion-mcp,
better-telegram-mcp) -- the same script, with the workspace image/service names.
Reads the gitignored ``wrangler.deploy.jsonc`` (real account/KV IDs) for the
...
Normal operation is CI: the ``deploy-cf`` job in .github/workflows/cd.yml runs
``--from-template`` on every release, so a released version and what is live on
Cloudflare are the same thing. Deploying by hand from a laptop breaks exactly
that property and is not the supported path.

For a local dry-run or an emergency redeploy, set CLOUDFLARE_API_TOKEN in the
environment and run from the repo root:
...
Requires: CLOUDFLARE_API_TOKEN in env, docker, and ``bunx wrangler``. The CF
container registry only pulls from registry.cloudflare.com/<account>/... (not
ghcr), so the local image is tagged to that path before ``wrangler containers
push``.

Why the rollout wait: a container keeps serving OLD Durable Object instances for
a while after deploy (``containers list`` STATE=provisioning); verifying during
that window shows stale behaviour.

Why the canary gate: the unit / T0 suite cannot reach the CF-glue layer
(worker.ts CONTAINER_ENV_SOURCES forwarding, container HOST bind, JWT signing
keys), which is exactly where the painful deploy regressions live. The gate is
credential-free (no Google client or per-user consent needed) and catches the
three that have actually happened on the sibling servers: (A)
``MCP_RELAY_PASSWORD`` dropped from the forward list -> ``/authorize`` serves the
delegated-OAuth start with no login front door = open self-service relay; (B) the
bearer gate dropped -> ``/mcp`` no longer 401s; (C) ephemeral per-instance JWT
keys -> ``/.well-known/jwks.json`` ``kid`` changes between requests (the
/token-signs-with-A, /mcp-verifies-with-B -> 401 bug), which for this server means
``CREDENTIAL_SECRET`` failed to reach the container and mcp-core's JWTIssuer fell
back to a key on the ephemeral container filesystem. On failure the previously
deployed image tag is redeployed automatically.

What the gate does NOT cover: the authenticated round-trip (relay login -> Google
consent -> ``config(account_add)`` -> a real domain tool call). That needs the Web
OAuth client plus a human at a Google consent screen, so it stays a manual smoke.
"""
...
def _load_deploy_config(repo: Path) -> dict:
    ...
    if not path.exists():
        sys.exit(
            ...
            f"render it with --from-template, or reconstruct it from the committed "
            f"{TEMPLATE_CONFIG} + CF resource IDs first."
        )
...
def _retry_gate(fn, public_url: str, *, tries: int = 10, delay: int = 6) -> bool:
    ...
    app is already serving. This image constructs all eleven Google services
    eagerly at module load (tools/domains/factory.ts), each pulling a generated
    googleapis surface, so it needs a good few seconds after ready before
    /authorize fronts /login -- a single check-once gate false-fails and triggers
    an unnecessary rollback. Mirror the settle-window approach _gate_kid_stable
    already uses."""
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.no_canary:
        print(f"DONE (--no-canary): {worker} deployed at tag {tag}. Verify manually.")
        return 0

    if _canary(_public_url(cfg), dry=args.dry_run):
        print(
            f"DONE: {worker} deployed at tag {tag}, canary PASS. The authenticated "
            "round-trip (relay login -> Google consent -> account_add -> tool call) "
            "is still a manual smoke."
        )
        return 0
