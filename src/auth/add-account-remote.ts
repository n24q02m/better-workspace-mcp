...
function formatCliCommands(text: string): string {
  return text.replace(/config\([^)]+\)/g, (match) => `<code>${match}</code>`)
}

const CALLBACK_CSS =
  'body{font-family:system-ui,sans-serif;max-width:34rem;margin:4rem auto;padding:0 1rem;line-height:1.5;text-align:center}main{display:flex;flex-direction:column;align-items:center}.main-content{display:flex;flex-direction:column;align-items:center}.feedback-icon{width:3rem;height:3rem;margin-bottom:1rem}.feedback-icon--success{color:#10b981}.feedback-icon--error{color:#ef4444}h1{margin-top:0}code{font-family:ui-monospace,monospace;font-size:0.875em;background-color:rgba(128,128,128,0.1);padding:0.2em 0.4em;border-radius:0.25rem}'
...
function respond(res: ServerResponse, status: number, title: string, detail: string): void {
  ...
  const safeDetail = formatCliCommands(escapeHtml(detail))
