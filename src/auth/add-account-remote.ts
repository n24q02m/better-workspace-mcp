...
function formatCommands(text: string): string {
  // Format config(action="...") as code blocks for better readability.
  return text.replace(/config\(action=&quot;([^&]+)&quot;\)/g, '<code>config(action=&quot;$1&quot;)</code>')
}

const CALLBACK_CSS =
  'body{font-family:system-ui,sans-serif;max-width:34rem;margin:4rem auto;padding:0 1rem;line-height:1.5;text-align:center}main{display:flex;flex-direction:column;align-items:center}.main-content{display:flex;flex-direction:column;align-items:center}.feedback-icon{width:3rem;height:3rem;margin-bottom:1rem}.feedback-icon--success{color:#10b981}.feedback-icon--error{color:#ef4444}h1{margin-top:0}code{background-color:rgba(128,128,128,0.15);padding:0.2em 0.4em;border-radius:0.25rem;font-family:ui-monospace,monospace;font-size:0.9em}'
...
function respond(res: ServerResponse, status: number, title: string, detail: string): void {
  ...
  const escapedDetail = formatCommands(escapeHtml(detail))
