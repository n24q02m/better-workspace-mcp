...
function respond(res: ServerResponse, status: number, title: string, detail: string): void {
  ...
  const safeDetail = formatMarkdown(escapeHtml(detail))
  const body = `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><meta name="color-scheme" content="light dark"><title>${safeTitle}</title><style>${CALLBACK_CSS}</style><body><main><div role="${role}" class="main-content">${icon}<h1>${safeTitle}</h1><p>${safeDetail}</p></div></main></body></html>`
