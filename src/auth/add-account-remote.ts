...
function formatInstructions(safeText: string): string {
  return safeText.replace(/(config\([^)]+\))/g, '<code>$1</code>')
}

const CALLBACK_CSS =
  'body{font-family:system-ui,sans-serif;max-width:34rem;margin:4rem auto;padding:0 1rem;line-height:1.5;text-align:center}main{display:flex;flex-direction:column;align-items:center}.main-content{display:flex;flex-direction:column;align-items:center}.feedback-icon{width:3rem;height:3rem;margin-bottom:1rem}.feedback-icon--success{color:#10b981}.feedback-icon--error{color:#ef4444}h1{margin-top:0}code{font-family:monospace;background:rgba(127,127,127,0.12);padding:0.2em 0.4em;border-radius:0.2em;font-size:0.9em}'
...
function respond(res: ServerResponse, status: number, title: string, detail: string): void {
  ...
  const safeDetail = formatInstructions(escapeHtml(detail))
